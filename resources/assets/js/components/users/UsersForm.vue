<template>
    <el-form :model="user" :rules="rules" class="margin-top" ref="userForm">
        ...
        <el-form-item prop="role" label="Choisir le rôle">
            <el-select v-model="user.role" placeholder="rôle de l'utilisateur" style="width: 100%">
                <el-option
                        v-for="role in roles"
                        :key="role.id"
...
<script>
    ...
    export default {
        props: ['user', 'validationError', 'roles'],
        components: {},
        data() {
        ...
        mounted() {
            if (this.$route.name === 'users:update') {
                ...
                this.rules.password_confirmation = null;
            }
        }
    }
</script>
