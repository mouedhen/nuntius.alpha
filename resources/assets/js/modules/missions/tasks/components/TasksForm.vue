    ...
    export default {
        ...
        data() {
            ...
            let validateEndDate = (rule, value, callback) => {
                let missionStart = moment(this.mission.start_date);
                let start = moment(this.task.start_date_time);
                let end = moment(this.task.end_date_time);
                if (start.isBefore(missionStart)) {
                    callback(new Error('La date de début de la tâche ne doit pas être inférieure à la date de début de la mission.'));
